...
class Spell_Checker:
    ...
    class Language_Model:
        ...
        def create_context_randomly(self, n):
            if self.n < n:
                first_ngram = random.choices(list(self.model_dict.keys()), weights=list(self.model_dict.values()), k=1)[0]
            ...
            if context == None:
                ...
                if len(words) >= n:
                    ...
                    if self.n > len(words): #not enough words in the first ngram
                        ...
                        if len(sub_dict) != 0:
                            return self.complete_context(n,random.choices(list(sub_dict.keys()), weights=list(sub_dict.values()), k=1)[0])
                        else:
                                if len(words) == 1:
                                    return context
                                else:
